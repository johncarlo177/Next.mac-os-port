<script lang="ts">
  ...
  function processCommand(cmd: string) {
    addLine(
      `<span class="text-green-400">johncarlo@macbook</span><span class="text-white">:</span><span class="text-blue-400">~/${currentDirectory.join("/")}</span><span class="text-white">$</span> ${cmd}`
    );
  ...
  onMount(() => {
    ...
    showPrompt = true;
    addLine("Welcome to John Carlo's Terminal");
...
<div class="bg-gray-900/40 backdrop-blur-md h-full border border-white/20">
  ...
  >
    ...
    <div class="flex-grow text-center text-sm text-gray-400">
      johncarlo@macbook: ~/{currentDirectory.join("/")}
    </div>
  </div>
  ...
  {#if showPrompt}
    <div class="bg-black/30 px-4 py-2 flex items-center">
      <span class="text-green-400 mr-1">johncarlo@macbook</span>
